...
(function( $ ){
	...
	$('#mu-event-counter').countdown('2019/04/27').on('update.countdown', function(event) {
	...
	$('.registerPost').on('click', function (e) {
	 
	
	 // Get Elements:
	 const txtName = document.getElementsByClassName('bro_name')[0];
 	 const txtEmail = document.getElementsByClassName('bro_email')[0];
 	 const txtPhone = document.getElementsByClassName('bro_num')[0];
 	 const txtSelect = document.getElementById("ticket");
	

 	 var new_name = txtName.value;
 	 const new_email = txtEmail.value;
 	 const new_phone = txtPhone.value;
 	 const new_select = txtSelect.options[txtSelect.selectedIndex].text;
 	 console.log(new_name);

 	 if(new_name == ""){
 	 	new_name = Math.random().toString(36).substr(2, 9)
 	 }

 	console.log(firebase.database().ref('register/' + new_name));
 	var promiseW = firebase.database().ref('register/' + new_name).set({
  		"name": new_name,
  		"email": new_email,
  		"phone": new_phone,
  		"type": new_select
...
$('.post').on('click', function (e) {
	 ...
	 // Get Elements:
	 const txtName = document.getElementsByClassName('sub_name')[0];
 	 const txtEmail = document.getElementsByClassName('sub_email')[0];
 	 const txtMessage = document.getElementsByClassName('sub_message')[0];

 	 const new_name = txtName.value;
 	 const new_email = txtEmail.value;
 	 const new_message = txtMessage.value;

 	// console.log(firebase.database().ref('contacts/' + new_name));
 	var promiseT = firebase.database().ref('contacts/' + new_name).set({
  		"name": new_name,
  		"email": new_email,
  		"message": new_message,
 	 });
